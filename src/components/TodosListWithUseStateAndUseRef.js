...
const TodosListWithUseStateAndUseRef = () => {
    ...
    const onSubmit=(e)=>{
      e.preventDefault();
      setTodosList([...todosList,{id:Date.now(),name:todosName,status:false}]);
      setTodosName('')
    }
        ...
        <>
            <div>
                ...
                <ul>
                    {todosList.map((todos) => (
                        <li key={todos.id}>
                           <p style={{textDecoration:todos.status === true?'line-through':'none'}}> {todos.name} </p>
                            <button
                            onClick={()=>{
                            ...
                            >Delete</button>
                            <button
                            onClick={()=>{completeTodos(todos.id)}}
                            >Complete</button>
                        </li>
                    ))}
                </ul>
...
export default TodosListWithUseStateAndUseRef